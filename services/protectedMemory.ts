...
function ProtectedMemory() {
  ...
  async function getCryptoKey() {
    ...
    const cryptoKey = await globalThis.crypto.subtle.importKey('raw', deSerialized, AES.name, false, [
      'encrypt',
      'decrypt',
    ]);
    return cryptoKey;
  ...
  async function getData(key: string) {
    ...
    return getCryptoKey()
      .then((cryptoKey) => {
        ...
        return globalThis.crypto.subtle.decrypt(AES, cryptoKey, encBytes);
      })
      .then(function (data) {
  ...
  function setData(key: string, data: any) {
    ...
    return getCryptoKey()
      .then((cryptoKey) => {
        return globalThis.crypto.subtle.encrypt(AES, cryptoKey, dataBytes);
      })
